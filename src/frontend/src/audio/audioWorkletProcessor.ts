// Exported as a string for use with URL.createObjectURL
// This code runs inside an AudioWorkletGlobalScope — no imports allowed.
...
class SpectralPaintProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    for (let i = 0; i < this.MAX_VOICES; i++) {
      const voice = {
        ...
        // ADSR
        adsrPhase: 0, // 0=attack,1=decay,2=sustain,3=release,4=off
        adsrLevel: 0,
        adsrAttack: 0.01,
        adsrDecay: 0.1,
        adsrSustain: 0.7,
    ...
    this.tempBuf = new Float32Array(128);

    // Effects: Chorus
    this.chorusEnabled = false;
    this.chorusMix = 0.3;
    this.chorusBuffer = new Float32Array(8192);
    ...
    this.lfoRate = 1.0;
    this.lfoDepth = 0;
    this.lfoWave = 0; // 0=sine,1=tri,2=square,3=random
    this.lfoTarget = 0; // 0=pitch,1=tension,2=cutoff,3=specTilt,4=bodyMix
    this.lfoValue = 0;
    this.lfoRandPrev = 0;
    this.lfoRandNext = 0;
  ...
  _triggerVoice(bins, pan, excitationType, amplitude, note) {
    ...
    // Calculate delay length from tension and note
    ...
    // Load excitation into delay buffer
  ...
  _generateExcitation(v, bins, excitationType) {
    ...
    // bins is an array of {bin, amplitude} values
    if (!bins || bins.length === 0) {
      // Default noise excitation
      for (let i = 0; i < len; i++) {
    ...
    // Build excitation waveform from bins
  ...
  _updateVoiceBody(v) {
    // Simple resonant filter for body modeling
    const bodyFreqs = [200, 180, 300, 250, 400, 350]; // Hz per body model
  ...
  _updateADSR(v, numSamples) {
    // Returns approximate level (simplified)
  ...
  process(inputs, outputs, parameters) {
    ...
    const pickup = parameters.pickupPosition[0];
